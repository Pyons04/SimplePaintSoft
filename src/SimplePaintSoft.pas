...
begin
  ...
  if IsDragging(p) and (mouse_X <= 723) then
    begin
      i := High(dots);
      FillCircle(selectedColor, mouse_X, mouse_Y, 5);
      dots[i].x := mouse_X;
      dots[i].y := mouse_Y;
     ...
     begin
      ...
      begin
         ...
         circles[i].radius := Sqrt(circles[i].radius)/2;
          ...
          FillCircle(circles[i].circleColor, circles[i].centerX, circles[i].centerY, castRadius);
          //WriteLn(castRadius,' is radius.');
      ...
      begin
        ...
        //    shapes[i].width := 1;
         ...
         begin
          ...
          if (castHight <> 0) and (castWidth <> 0) then
           FillRectangle(shapes[i].shapeColor, shapes[i].x,shapes[i].y,castWidth,castHight);
   
         WriteLn('shape ',i, 'has ',castWidth,' width.');
         WriteLn('shape ',i, 'has ',castHight,' hight.');
